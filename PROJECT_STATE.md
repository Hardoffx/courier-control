...
Последнее обновление: 2026-09-20. Repo: Hardoffx/courier-control. Branch: main.
...
- Courier reorder now swaps only the two affected unfinished positions; completed and unrelated rows keep their exact route_order, preventing list reshuffles. The moved point remains selected after the action.
- Dispatcher next-stop selection prefers an unfinished non-problem point, matching courier selection semantics.
- Deferred map monitoring remains explicitly out of scope for the current update.
