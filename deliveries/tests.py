...
class DeliveryWorkflowTests(TestCase):
    ...
    def test_completion_with_gps_is_recorded(self):
        self.delivery.courier=self.courier; self.delivery.save(); self.client.login(username='courier',password='pass'); self.client.post(reverse('courier_update',args=[self.delivery.pk]),{'action':'done','latitude':'55.751244','longitude':'37.618423'}); self.delivery.refresh_from_db(); self.assertEqual(str(self.delivery.completed_latitude),'55.751244'); self.assertTrue(self.delivery.events.filter(action='done',note__icontains='GPS').exists())
    def test_problem_preset_and_comment(self):
        self.delivery.courier=self.courier; self.delivery.save(); self.client.login(username='courier',password='pass'); self.client.post(reverse('courier_update',args=[self.delivery.pk]),{'action':'problem','problem_reason':'Нет доступа','problem_comment':'охрана не пустила'}); self.delivery.refresh_from_db(); self.assertIn('Нет доступа',self.delivery.problem_reason); self.assertIn('охрана',self.delivery.problem_reason)
    def test_history_is_dispatcher_only(self):
        DeliveryEvent.objects.create(delivery=self.delivery,actor=self.dispatcher,action='created',note='test'); self.client.login(username='dispatcher',password='pass'); self.assertEqual(self.client.get(reverse('delivery_history',args=[self.delivery.pk])).status_code,200); self.client.login(username='courier',password='pass'); self.assertEqual(self.client.get(reverse('delivery_history',args=[self.delivery.pk])).status_code,403)
...
class RouteTemplateTests(TestCase):
    ...
    def test_copy_previous_route_keeps_today_courier(self):
        yesterday=timezone.localdate()-timedelta(days=1); old=generate_route_run(self.weekday,yesterday,courier=self.primary); extra=DeliveryPoint.objects.create(name='Extra',address='Москва, C'); Delivery.objects.create(delivery_date=yesterday,route_run=old,point=extra,source_label='Extra',address='Москва, C',courier=self.primary,route_order=3); today=generate_route_run(self.weekday,timezone.localdate(),courier=self.reserve); self.client.login(username='route-dispatcher',password='pass'); self.client.post(reverse('run_copy_previous',args=[today.pk])); self.assertEqual(today.deliveries.count(),3); self.assertFalse(today.deliveries.exclude(courier=self.reserve).exists())
