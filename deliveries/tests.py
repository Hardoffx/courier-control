...
class RouteTemplateTests(TestCase):
    ...
    def test_weekday_and_weekend_are_independent(self):
        self.assertEqual(list(self.weekday.items.values_list('point_id',flat=True)),[self.a.id,self.b.id]); self.assertEqual(list(self.weekend.items.values_list('point_id',flat=True)),[self.b.id]); self.assertEqual(self.weekend.items.first().time_window,'14:00')
    def test_generation_uses_default_courier_and_canonical_point_data(self):
        run=generate_route_run(self.weekday,timezone.localdate()); self.assertEqual(run.assigned_courier,self.primary); self.assertEqual(run.deliveries.count(),2); first=run.deliveries.order_by('route_order').first(); self.assertEqual(first.address,'Москва, A'); self.assertEqual(first.courier,self.primary); self.assertEqual(first.time_window,'09:00')
    def test_reserve_courier_can_take_day_without_changing_template_owner(self):
        run=generate_route_run(self.weekday,timezone.localdate()); reassign_route_run(run,self.reserve); run.refresh_from_db(); self.route.refresh_from_db(); self.assertEqual(run.assigned_courier,self.reserve); self.assertEqual(self.route.default_courier,self.primary); self.assertFalse(run.deliveries.exclude(courier=self.reserve).exists())
    def test_regeneration_does_not_duplicate_points(self):
        run=generate_route_run(self.weekday,timezone.localdate()); generate_route_run(self.weekday,timezone.localdate()); self.assertEqual(RouteRun.objects.count(),1); self.assertEqual(run.deliveries.count(),2)
    def test_regeneration_preserves_completed_delivery(self):
        run=generate_route_run(self.weekday,timezone.localdate()); done=run.deliveries.get(point=self.a); done.status=Delivery.Status.DONE; done.save(); only_b=[self.weekday.items.get(point=self.b).id]; generate_route_run(self.weekday,timezone.localdate(),enabled_item_ids=only_b); self.assertTrue(Delivery.objects.filter(pk=done.pk,status=Delivery.Status.DONE).exists())
