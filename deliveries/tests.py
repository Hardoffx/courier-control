...
class DeliveryWorkflowTests(TestCase):
    ...
    def test_courier_phone_edit_persists_on_point_and_all_days(self):
        point=DeliveryPoint.objects.create(name='Точка',address='Москва, Тестовая 1',phone='')
        self.delivery.point=point; self.delivery.courier=self.courier; self.delivery.save(update_fields=['point','courier','updated_at'])
        tomorrow=Delivery.objects.create(delivery_date=timezone.localdate()+timedelta(days=1),point=point,address=point.address,phone='',route_order=1)
        yesterday=Delivery.objects.create(delivery_date=timezone.localdate()-timedelta(days=1),point=point,address=point.address,phone='+70000000000',status=Delivery.Status.DONE,route_order=1)
        self.client.login(username='courier',password='pass')
        response=self.client.post(reverse('courier_update',args=[self.delivery.pk]),{'action':'phone','phone':'+7 999 123-45-67'})
        self.assertEqual(response.status_code,302)
        point.refresh_from_db(); self.delivery.refresh_from_db(); tomorrow.refresh_from_db(); yesterday.refresh_from_db()
        self.assertEqual(point.phone,'+7 999 123-45-67')
        self.assertEqual(self.delivery.phone,point.phone)
        self.assertEqual(tomorrow.phone,point.phone)
        self.assertEqual(yesterday.phone,point.phone)
...
class RouteTemplateTests(TestCase):
    ...
    def test_route_list_renders_route_without_courier(self):
        self.route.default_courier=None; self.route.save(update_fields=['default_courier']); self.client.login(username='route-dispatcher',password='pass'); response=self.client.get(reverse('route_list')); self.assertEqual(response.status_code,200); self.assertContains(response,'Не назначен')
    def test_route_detail_renders_route_without_courier(self):
        self.route.default_courier=None; self.route.save(update_fields=['default_courier']); self.client.login(username='route-dispatcher',password='pass'); response=self.client.get(reverse('route_detail',args=[self.route.pk])); self.assertEqual(response.status_code,200); self.assertContains(response,'Не назначен')
    def test_route_can_be_created_without_couriers(self):
        User.objects.filter(role=User.Role.COURIER).delete(); self.client.login(username='route-dispatcher',password='pass'); response=self.client.post(reverse('route_create'),{'name':'Без курьера','default_courier':'','notes':''}); self.assertEqual(response.status_code,302); self.assertIsNone(Route.objects.get(name='Без курьера').default_courier)
