...
def run_detail(request,pk):
    run=get_object_or_404(RouteRun.objects.select_related('route','template','assigned_courier'),pk=pk); deliveries=run.deliveries.select_related('point').prefetch_related('events__actor').order_by('route_order','id'); points=DeliveryPoint.objects.filter(is_active=True).order_by('kind','name'); previous=RouteRun.objects.filter(route=run.route,run_date__lt=run.run_date).order_by('-run_date').first(); templates=run.route.templates.filter(is_active=True).order_by('kind','id'); return render(request,'dispatcher/route_run_detail.html',{'run':run,'deliveries':deliveries,'couriers':_couriers(),'points':points,'previous_run':previous,'templates':templates})
@dispatcher_required
@require_POST
...
def run_learn_template(request,pk):
    run=get_object_or_404(RouteRun,pk=pk); template=get_object_or_404(RouteTemplate,pk=request.POST.get('template_id'),route=run.route,is_active=True)
    try: count=learn_template_from_run(run,template)
    except ValueError as exc: messages.error(request,str(exc)); return redirect('run_detail',pk=pk)
    messages.success(request,f'Шаблон «{template}» обновлён по этому дню: {count} точек. Другие шаблоны не изменены.'); return redirect('run_detail',pk=pk)
@dispatcher_required
@require_POST
